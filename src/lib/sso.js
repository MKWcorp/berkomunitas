...
export async function loginWithGoogle(googleToken, platform = 'Berkomunitas') {
  ...
  if (typeof window !== 'undefined') {
    ...
    document.cookie = `access_token=${data.accessToken}; path=/; max-age=${7 * 24 * 60 * 60}`; // 7 days
    document.cookie = `refresh_token=${data.refreshToken}; path=/; max-age=${30 * 24 * 60 * 60}`; // 30 days
  }
